require 'rails_helper'
  describe StaffMember do
    describe '#password' do
      example '文字を与えるとhashed_passwordは長さ60の文字列になる' do
        member = StaffMember.new
        member.password = 'baukis'
        expect(member.hashed_password).to be_kind_of(String)
        expect(member.hashed_password.size).to eq(60)
      end
      example 'nilを与えるとhash_passwordはnilになる' do
    ...
    describe '値の正規化' do
      ...
      example 'family_name_kanaに含まれる半角カタカナをカタカナに変換' do
    ...
    describe 'バリデーション' do
      ...
      example '長音符を含むfamily_name_kanaは無効' do
      ...
      example 'family_nameは漢字,ひらがな,カタカナ,アルファベット以外の文字を含まない' do
        member = build(:staff_member, family_name: 'あご★')
        expect(member).not_to be_valid
      end

      example 'given_nameは漢字,ひらがな,カタカナ,アルファベット以外の文字を含まない' do
        member = build(:staff_member, given_name: 'あご★')
        expect(member).not_to be_valid
      end
    end
  end
